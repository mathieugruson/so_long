...
void	get_filename(t_mlx	*mlx)
{
	...
	mlx->filename_s = "./img/player_lifedraft_cap.xpm";
	mlx->filename_w = "./img/player_raft.xpm";
...
void	get_xpm_file(t_mlx *mlx)
{
	...
	mlx->img_w = mlx_xpm_file_to_image(mlx->mlx_ptr, mlx->filename_w, &mlx->img_size,&mlx->img_size);
}
...
char	display_at_window(char **map, t_mlx *mlx)
{
	t_xy	map_size;
	int		KeySym;
